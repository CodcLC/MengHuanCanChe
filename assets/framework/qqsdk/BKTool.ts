...
export default class BKTool
{
    ...
    static shareToArk() {
        BK.Share.share({
            ...
            success: function (succObj) {
                cc.log('分享成功', succObj.code, JSON.stringify(succObj.data));
            },
            fail: function (failObj) {
                cc.log('分享失败', failObj.code, JSON.stringify(failObj.msg));
            },
            complete: () => {
                cc.log('分享完成，不论成功失败');
            }
    ...
    static follow() {
        cc.log("Global.PUIN>" + Global.PUIN);
    ...
    saveGameData(data){
        ...
        return new Promise((resolve)=>{
            ...
            BK.QQ.saveGameData(data, (errCode, cmd, data)=>{
                BK.cc.log(1, 1, 'saveGameData : ' + errCode + ', ' + cmd + ', ' + data);
    ...
    loadGameData(){
        return new Promise((resolve)=>{
            BK.QQ.loadGameData((errCode, cmd, data)=> {
                ...
                BK.cc.log(1, 1, 'loadGameData : ' + errCode + ', ' + cmd + ', ' + data);
                resolve({errCode:errCode, cmd:cmd, data:data});
            });
        })
    }
    ...
    */
    ...
    static test(){
		...
		// 	cc.log("[fetchOpenKey]",errCode, cmd, data)
		//     if (errCode == 0) {
		...
		BK.cc.log('游戏版本号:'+systemInfo.gameVersion);
		BK.cc.log('是否房主:'+systemInfo.isMaster);   //使用厘米秀房间时才有效
		BK.cc.log('房间号:'+systemInfo.roomId); //使用厘米秀房间时才有效
		BK.cc.log('系统版本:'+systemInfo.osVersion);
		BK.cc.log('网络类型:'+systemInfo.networkType);

		BK.cc.log('平台:'+systemInfo.platform);
		BK.cc.log('当前用户的标识:'+systemInfo.openId);
		BK.cc.log('手机qq版本:'+systemInfo.QQVer);

		BK.cc.log('是否首次安装:'+systemInfo.isFirstInstall);
		BK.cc.log('当前聊天窗类型:'+systemInfo.aioType);
		BK.cc.log('游戏启动入口:'+systemInfo.src);
		BK.cc.log('是否为管理账号:'+systemInfo.isWhiteUser);
		BK.cc.log('游戏类型:'+systemInfo.gameType);
		BK.cc.log('具体机型:'+systemInfo.model);
		BK.cc.log('性别:'+systemInfo.sex);
    ...
    static uploadScore(score, callback?) {
        ...
        BK.QQ.uploadScoreWithoutRoom(1, data, function (errCode, cmd, data) {
            cc.log("-------------uploadScoreWithoutRoom callback  cmd" + cmd + " errCode:" + errCode + "  data:" + JSON.stringify(data));
            if (callback) {
    ...
    static getRankList(callback) {
        ...
        BK.QQ.getRankListWithoutRoom(attr, order, rankType, function (errCode, cmd, data) {
            cc.log("--------------------getRankListWithoutRoom callback  cmd" + cmd + " errCode:" + errCode);
            if (errCode != 0) {
            ...
            if (data) {
                ...
                cc.log("=====data not null " + rankList.length);
    ...
    static loadVideoAd(callback) {
        cc.log("============BKTOOl.loadVideoAD" , Global.videoAd);
        
        // if (!Global.videoAd ) { //如果没有广告资源就加载新的视频广告
        let videoAd = BK.Advertisement.createVideoAd();
        videoAd.onError(function (err) {
            //加载失败
            cc.log(">>>>>>>>>BKTools onError code:" + err.code + " msg:" + err.msg);
        ...
        //  else {
        //     cc.log("BKTools 已存在广告资源 或者 非QQ玩一玩平台....");
        // }
    ...
    static loadBannerAd(callback?, viewId?) {
        ...
        cc.log("loadBannerAd viewId:" + viewId);
    ...
    static showBannerAd() {
        cc.log("BKTools 显示banner广告")
        if (Global.bannerAd) {
            Global.bannerAd.show();
        } else {
            cc.log("BKTools 不存在banner资源....");
    ...
    static hideBannerAd() {
        if (Global.bannerAd) {
            ...
            cc.log("BKTools 不存在banner资源无法关闭....");
        }
    }
    ...
    static showToast(title, duration) {
        ...
        if (cc.sys.platform == cc.sys.QQ_PLAY) {
            BK.UI.showToast({
                ...
                duration: duration,
                complete: function () {
                    cc.log("BKTools showToast complete:" + title);
                }
            });
        }
    }
